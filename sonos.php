...
class Sonos
{
  ...
  function getMetadata($params)
  {
    ...
    if ($id == "root") {
      ...
      foreach (
        array_slice($podcast->episodeIDs, $index, $count)
        as $episodeID
      ) {
        $mediaMetadata[] = $this->findEpisodeMediaMetadata(
          $episodeID,
          in_array($episodeID, $activeEpisodeIDs)
        );
    ...
    $response->getMetadataResult->count =
      count($mediaCollection) + count($mediaMetadata);
  ...
  function getMediaMetadata($params)
  {
    ...
    $activeEpisodeIDs = fetchAccount($this->sessionId)->episodeIDs;
    $response->getMediaMetadataResult = $this->findEpisodeMediaMetadata(
      $id,
